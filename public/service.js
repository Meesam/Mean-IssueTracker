...
function appServices($http, $q, $rootScope) {
    ...
    function doLogin(lInfo) {
        ...
        let request = $http({ method: 'post', url: '/api/login', data: lInfo });
        return (request.then(handleSuccess, handleError));
    }
    function doActionPost(actionParam, path) {
        NProgress.start(); $rootScope.isBusy += 1;
        let request = $http({ method: 'post', url: '/api/' + path, data: actionParam });
        return (request.then(handleSuccess, handleError));
    }
    function doActionGet(actionParam, path) {
        NProgress.start(); $rootScope.isBusy += 1;
        let request = $http({ method: 'GET', url: '/api/' + path, data: actionParam });
        return (request.then(handleSuccess, handleError));
    }
    function doSearch(aSearch) {
        NProgress.start(); $rootScope.isBusy += 1;
        let request = $http({ method: 'get', url: 'api/search/global?Token=' + aSearch.Token + '&Search=' + aSearch.Value });
        return (request.then(handleSuccess, handleError));
    }
    ...
    function getTable(aTabInfo) {
        NProgress.start(); $rootScope.isBusy += 1;
        let request = $http({ method: 'POST', url: '/api/' + aTabInfo.api, data: aTabInfo });
        return (request.then(handleSuccess, handleError));
    }
    function getUserByToken(userToken) {
        $rootScope.isBusy += 1;
        let request = $http({ method: 'GET', url: '/api/userfromtoken', data: { 'Token': userToken } });
        return (request.then(handleSuccess, handleError));
    }
    function chPW(userToken, oldPW, newPW) {
        NProgress.start(); $rootScope.isBusy += 1;
        let request = $http({ method: 'post', url: 'api/changepw', data: { 'Token': userToken, 'Value': oldPW, 'Data': newPW } });
        return (request.then(handleSuccess, handleError));
    }
